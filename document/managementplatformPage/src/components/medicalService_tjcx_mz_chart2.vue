<template>
    <div class="medicalService_tjcx_mz_chart2">
    ...
    methods:{
        ...
        async initChart(){
            ...
            let option = {
                ...
                legend: {
                    ...
                    // data:['挂号','诊察','检查','化验','治疗','手术','药品','材料','其他']
                },
                series: [{
                    name:this.name,
                    type:'pie',
		...
		async fyxq(param,name){
            ...
            this.list =res.data[0]; 
                this.info.fylb = name
                this.info.fy= Number(this.list.MZFY_)
                this.info.hb = Number(this.list.MZFY_MOMGROWTH)/Number(this.list.MZFY_MOM)
                this.info.tb = Number(this.list.MZFY_YOYGROWTH)/Number(this.list.MZFY_YOY)  
        },        
		async init(){
			let data = {
                "themeID":this.CWorYW+"_flfy_tj",
                "startdate":this.startTime_show,
                "enddate":this.endTime_show, 
				// "startdate":"2019-01-01",
				// "enddate":"2019-01-11"			
